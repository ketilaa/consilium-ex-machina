...
def run_concur_recheck_round2_only(scenario, original_concern, after_decision):
    """Round 2 in isolation, given a FIXED original_concern (not freshly
    generated by round 1) -- tests round 2's negative case: does it correctly
    still say DO NOT CONCUR when `after_decision` does NOT resolve
    `original_concern`? Every prior test of round 2 (run_concur_recheck) paired
    a real concern with a revision engineered to resolve it, 9/9 CONCUR --
    this is the missing other half. Reuses concur_recheck_system unmodified;
    only the concern's origin (fixed text vs. a fresh round-1 call) differs."""
    role = scenario["concur_role"]
    grounds = scenario["concur_grounds"]
    verdict = chat(
        SUPPORT_MODEL,
...
def run_concur_recheck_round2_repeats(scenario, original_concern, after_decision, n=3):
    verdicts = []
    concurs = []
    for _ in range(n):
        verdict, concurs_flag = run_concur_recheck_round2_only(scenario, original_concern, after_decision)
        verdicts.append(verdict)
        concurs.append(concurs_flag)
    return verdicts, concurs
